...
type TemplateLabels = Record<string, string>
...
export function stripScenarioPromptEnvelope(content: string): string {
...
export function hasScenarioPromptEnvelope(content: string): boolean {
  const text = String(content || '')
...
export function resolveUserMessageDisplayContent(
  rawMessage: PersistedUserMessage | null | undefined,
...
export function composeScenarioPrompt(message: string, template?: ScenarioTemplate | null): string {
...
export function resolveTemplateBubbleText(templateLabel: string, fallbackText = ''): string {
