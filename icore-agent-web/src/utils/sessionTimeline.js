...
export function normalizeTimelineItem(input) {
...
export function normalizeTimelineTurn(input) {
  return {
    ...
    items: (input?.items || []).map(normalizeTimelineItem).filter((item) => item.itemId),
  }
}

/** Hydrate the canonical session timeline returned by the backend session state API. */
export function hydrateSessionTimeline(state = {}) {
  return {
    sessionId: String(state.session_id || state.sessionId || '').trim(),
    summary: state.summary || null,
    turns: (state.turns || []).map(normalizeTimelineTurn).filter((turn) => turn.turnId),
...
export function upsertTimelineItem(turn, itemInput) {
  ...
  const index = turn.items.findIndex((existing) => existing.itemId === item.itemId)
  if (index >= 0) {
...
export function applyTurnEvent(timeline, event) {
  ...
  if (type === 'turn_completed') {
    ...
    const completed = findTimelineTurn(timeline, turnId)
  ...
  if (type === 'turn_failed') {
    replaceTurnIfProvided(timeline, turnId, event.turn)
    const failed = findTimelineTurn(timeline, turnId)
  ...
  if (type === 'turn_aborted') {
    replaceTurnIfProvided(timeline, turnId, event.turn)
    const aborted = findTimelineTurn(timeline, turnId)
...
export function timelineToChatRows(timeline, options = {}) {
...
export function userMessageText(payload) {
...
function findTimelineTurn(timeline, turnId) {
  return timeline.turns.find((turn) => turn.turnId === turnId)
}

/** Return an existing turn or append a new in-progress turn. */
function ensureTimelineTurn(timeline, turnId) {
...
function replaceTurnIfProvided(timeline, turnId, rawTurn) {
...
function appendTimelineItemDelta(turn, event) {
...
function userMessageRow(turn, item, lookup, assignedUuids, templateLabels) {
...
function agentMessageRow(turn, item, toolSteps) {
...
function contextDebugRow(turn, item) {
  return {
    id: `${turn.turnId}-${item.itemId}`,
    turnId: turn.turnId,
    itemId: item.itemId,
...
function toolCallStep(item, index) {
