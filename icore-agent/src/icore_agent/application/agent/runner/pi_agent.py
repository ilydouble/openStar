"""Pi agent runner — proxies agent turns to the pi-source-service Node.js microservice.

PiAgentRunner implements the PreparedAgentRunner protocol so AgentLoop can
drive it the same way as the Strands orchestrator: runner.messages = history,
...
``tool_start`` and ``file_changed`` events are surfaced to the turn timeline
via ``StrandsToolEventBridge.on_callback`` (see
``icore_agent.application.agent.tool.event_bridge``), which understands the
``current_tool_use`` and ``file_change`` callback kwargs emitted below.
"""
...
class PiAgentRunner:
    """Synchronous runner that streams agent turns through pi-source-service."""
    ...
    def __call__(self, message: str) -> str:
        """Send one turn to pi-source-service and return the assembled reply."""
...
def create_pi_orchestrator(
    ...
    **_kwargs: Any,
    ...
    """Factory matching the OrchestratorFactory signature used by AgentTurnService."""
    system_prompt = (
