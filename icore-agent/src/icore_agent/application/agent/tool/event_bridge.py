...
class StrandsToolEventBridge:
    ...
    def on_callback(self, **kwargs: Any) -> None:
        ...
        if isinstance(file_change, dict):
            self.record_file_changed(file_change)
    ...
    def record_file_changed(self, change: dict[str, Any]) -> None:
        """Emit a synthetic completed tool-call item for a Pi mode file edit.

        Pi mode (pi-source-service) reports file writes/edits out-of-band via
        a ``file_change`` callback kwarg rather than a Strands tool-use cycle.
        We surface this to the frontend as a one-shot completed tool-call
        item carrying the full PendingChange record (changeId, path,
        commitHash, tool, bytes, changedAt, savedAt) in
        ``result.structured_content`` so the UI can render an Undo / Save All
        card for the affected path.
        """
        now = datetime.now(UTC)
        item = ToolCallItem(
            status=ToolCallStatus.COMPLETED,
            function=ToolFunction(
                name="pi_file_changed",
                arguments_text=json_dumps(change),
                arguments_json=json_safe_object(change),
            ),
            result=ToolCallResult(
                content=str(change.get("path") or ""),
                structured_content=json_safe_object(change),
            ),
            started_at=now,
            completed_at=now,
            duration_ms=0,
