...
def normalize_fact_key(raw_key: str) -> str:
    ...
    normalized = re.sub(r"[^a-z0-9_]+", "_", str(raw_key or "").strip().lower())
...
def turn_relevance_score(
    ...
    tokens = {token for token in re.findall(r"[a-z0-9\u4e00-\u9fff]+", haystack) if len(token) > 2}
    fact_tokens = {
        token
    ...
    boost = 0.15 if hint and fact.category in _HINT_CATEGORY_BOOST.get(hint, set()) else 0.0
