"""Tool-event bridge for direct Chat Completions tool calls."""
...
class ChatCompletionsToolEventBridge:
    """Convert direct tool execution callbacks into turn-item events."""
    ...
    def bound_to(
        ...
        *,
        ...
        """Bind synchronous event sinks for one Chat Completions run."""
        previous = self._emit
    ...
    def on_callback(self, **kwargs: Any) -> None:
        """Observe streaming callbacks from runner or test fakes."""
    ...
    def record_start(self, tool_use: dict[str, Any]) -> None:
        """Emit a tool-call start item once per provider tool-use id."""
...
def create_chat_completions_tool_event_bridge(
    *,
    session_id: str,
    turn_id: str,
) -> ChatCompletionsToolEventBridge:
    """Create a Chat Completions tool event bridge for one agent turn."""
    return ChatCompletionsToolEventBridge(
