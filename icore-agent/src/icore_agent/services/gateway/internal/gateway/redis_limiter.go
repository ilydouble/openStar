...
func (limiter *RedisLimiter) Allow(ctx context.Context, service string) (RateLimitDecision, error) {
	if limiter.limit <= 0 {
		return RateLimitDecision{Allowed: true, Result: "disabled"}, nil
	}
	...
	return RateLimitDecision{Allowed: true, Result: "allowed"}, nil
}
